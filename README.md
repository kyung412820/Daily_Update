...
이 프로젝트는 간단한 일정 관리 애플리케이션으로, 사용자가 일정 데이터를 입력하고 관리할 수 있는 기능을 제공합니다. 추가로, 게시물 생성 및 조회, 수정, 삭제와 같은 기본적인 CRUD 기능도 구현되었습니다. 사용자 친화적인 UI와 직관적인 사용 방법을 통해 누구나 쉽게 활용할 수 있습니다. 이 애플리케이션은 Java Spring Framework를 기반으로 개발되었으며, Git을 사용해 버전 관리가 이루어졌습니다. 다만 전과 다른 점이 있다면 jpa를 주력으로 사용했다는 점과 쿠키와 세션을 통해 로그인을 구현하였습니다.
...
로그인 구현:
세션과 쿠키를 통하여 id,pw를 통한 로그인을 구현했습니다.
...
#### 3. 데이터베이스 테이블
게시물 데이터를 저장하기 위해 두 개의 테이블(user, schedule)을 사용했습니다.
